...
export function subscriptionColumns(studentId: string): Column<SubscriptionRow>[] {
  return [
    ...
    },
    {
      ...
      render: (r) =>
        r.status === 'ملغي' ? (
          <ButtonLink to={`/students/${studentId}/open-course`} variant="primary" size="sm">
            {r.action}
          </ButtonLink>
        ) : (
          <ButtonLink
...
export function StudentDetailBody({
  ...
  if (loading) return <CardSkeleton className="h-[400px]" />
  if (error || !student) {
  ...
  return (
    <>
      ...
      <StudentProfileCard student={student} onBan={onBan} />
