...
|---|---|---|
| 2026-08-31 | **رد فعلي على رسايل الزوار من داخل الداشبورد** — المستخدم جرّب فيتشر الدمج (الصف اللي فات) ولقى إن زرار الرد بيفتح mailto: خارج التطبيق، وطلب إنه يكتب الرد عادي جوه الداشبورد زي أي تذكرة تانية ويتبعت إيميل على طول. زرار «الرد بالإيميل» بقى بيفتح مودال (`GuestContactReplyModal`) فيه نص الرسالة الأصلية + خانة كتابة، وبيبعت `POST /contact-support/:id/reply` (جديد في الباك اند — بيبعت إيميل حقيقي عبر `MailService.sendContactSupportReply` ويسجل `replyMessage`/`repliedAt`/`repliedByAdminName` على الرسالة). حالة الصف في الجدول بقت «تم الرد» (تون أخضر) بعد الرد، بدل «رسالة تواصل» الثابتة | `api/tickets.ts` · `data/tickets.ts` · `pages/Support/tickets-parts.tsx` · `pages/Support/TicketsScreen.tsx` — **باك اند** (`D:\Step-Backend`): `contact-support/dto/reply-contact-support-message.dto.ts` (جديد) · `contact-support.service.ts` (+ `.spec.ts` جديد) · `contact-support.controller.ts` · `mail.service.ts` · `database/entities/contact-support-message.entity.ts` · `tickets.service.ts` |
| 2026-08-31 | **دمج رسايل "تواصل مع الدعم" (زوار مش مسجلين) في نفس جدول تذاكر الدعم** — بالتنسيق مع الباك اند (طلب اتوضح من المستخدم بعد ما جرّب فيتشر contact-support ومالقاش رسايله في `/tickets`). `GET /tickets` بقى بيرجّع صف مدموج من مصدرين مختلفين تمامًا (`SupportTicket` و`ContactSupportMessage`، جدول منفصل بدون status/priority/category)، كل صف معلّم بـ `kind`: `STUDENT_TICKET` أو `GUEST_CONTACT`. صفوف الزوار بتظهر بس في تاب "الكل" (مفيش عندها status/priority/category تتفلتر بيه)، وعمود «الطالب» بيعرض بادج «زائر» زيادة على الاسم (بديل: الاسم لو اتكتب، وإلا الإيميل)، وزرار الإجراء بيفتح `mailto:` على إيميل الزائر بدل ما يودي لصفحة تفاصيل مش موجودة أصلًا لصفوف الزوار | `api/tickets.ts` · `data/tickets.ts` · `components/ui/Badge.tsx` · `pages/Support/tickets-parts.tsx` — **باك اند** (`D:\Step-Backend`): `tickets.module.ts` · `tickets.service.ts` (+ `tickets.service.spec.ts` جديد) |
| 2026-08-30 | **فلاتر الهيكل الأكاديمي (جامعة→كلية→تخصص) في الإشعارات + الطلاب + طلبات الشراء + التقارير الأربعة** — بالتنسيق مع الباك اند (`step-backend-3b`) اللي ضاف `universityId`/`collegeId`/`specializationId` كـ query params اختيارية على `/students`، `/orders`، و`/reports/revenue|students|orders` (`/reports/devices` بياخد نفس الشكل بس بيتجاهل الفلتر، مفيش علاقة كورس). كل صفحة فيها نفس الكاسكيد: اختيار الجامعة يصفّي قايمة الكليات، اختيار الكلية يصفّي التخصصات، وكل تغيير في مستوى أعلى بيصفّر المستويات اللي تحته. اتصلح كمان بالمرة **باج حقيقي**: فلتر الكورس في `/students` كان بيبعت `id` الكورس، لكن الباك اند بيفلتر `sub.courseName ILIKE` (مطابقة نصية بالاسم) — فالفلتر ماكانش بيشتغل خالص؛ اتصلح ليبعت اسم الكورس. وفي التقارير، دروب داون «تجميع بحسب: الكلية» كان شكلي بالكامل (خيار واحد ثابت، `onChange` فاضي) — اتشال واستُبدل بالفلتر الحقيقي الجديد | `api/students.ts` · `api/orders.ts` · `api/reports.ts` · `pages/Students/StudentsList.tsx` · `pages/Orders/OrdersScreen.tsx` · `pages/Reports/reports-parts.tsx` · `pages/Reports/ReportsOverview.tsx` · `ReportStudents.tsx` · `ReportOrders.tsx` · `data/students.ts` · `data/orders.ts` — **باك اند** (`D:\Step-Backend`): `students`/`orders`/`reports` DTOs + services (تعديل خارج هذه الجلسة) |
