...
## التوافقية والتجاوب (Responsive Design)

المشروع متجاوب بالكامل (Mobile-First). التزم بالقواعد التالية عند بناء شاشات أو مكونات جديدة:

1. **الترتيب الرأسي أولاً (Stacking):**
   استخدم `flex-col` كوضع افتراضي على الموبايل، وقم بتغييره إلى `sm:flex-row` أو `lg:flex-row` في الشاشات الأكبر. 
   مثال: `<div className="flex w-full shrink-0 flex-col items-start gap-4 lg:flex-row">`
2. **التمرير الأفقي (Horizontal Scroll):**
   أي جدول `DataTable` يجب أن يحتوي على `min-w-[800px]` (أو حسب المحتوى) لمنع انهيار التصميم أو انضغاط النصوص على الموبايل. الـ `Tabs` مجهزة افتراضياً للتمرير الأفقي.
3. **عرض النوافذ (Modals):**
   المكون الأساسي للمودال مُجهز ليكون متجاوباً (`w-[calc(100%-32px)]` للموبايل وأقصى عرض `max-w-[...]` للديسكتوب). لا تقم بإجبار المودال على عرض ثابت بالبكسل.
4. **شريط التنقل (Sidebar):**
   يتحول إلى نافذة منبثقة (Overlay Panel) أسفل عرض `1024px`، ولا يتم تصغيره أبداً.
