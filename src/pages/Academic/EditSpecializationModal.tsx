...
import { listSpecializations, updateSpecialization } from '@/api/academic'
...
export default function EditSpecializationModal() {
  ...
  useEffect(() => {
    if (specialization) {
      ...
      setActive(specialization.status === 'نشط')
    }
  }, [specialization])
  ...
  const handleConfirm = async () => {
    ...
    setSubmitError(null)
    try {
      await updateSpecialization(id, { name, status: active ? 'ACTIVE' : 'DISABLED' })
      onDataChanged()
      navigate(-1)
    } catch (err) {
  ...
  return (
      ...
      actions={
        <>
          ...
          <ModalButton
            ...
            disabled={submitting || !name.trim() || !specialization}
          >
    ...
    >
      {loading ? (
        ...
        <>
          <ModalField label="اسم التخصص" value={name} onChange={setName} />
          <ModalToggleRow
            label="الحالة"
