import { useEffect, useState } from 'react'
import { useNavigate, useOutletContext, useParams } from 'react-router-dom'
import {
  Modal,
  ModalButton,
  ModalDangerZone,
  ModalField,
  ModalToggleRow,
} from '@/components/ui/Modal'
import { CardSkeleton, ErrorState } from '@/components/ui/States'
import { useAsync } from '@/lib/useAsync'
import { listUniversities, updateUniversity } from '@/api/academic'

type AcademicOutletContext = { onDataChanged: () => void }

export default function EditUniversityModal() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { onDataChanged } = useOutletContext<AcademicOutletContext>()
  const { data, loading, error } = useAsync(() => listUniversities({ limit: 100 }), [])
  const university = data?.data.find((u) => u.id === id)

  const [name, setName] = useState('')
  const [active, setActive] = useState(true)
  const [submitting, setSubmitting] = useState(false)
  const [submitError, setSubmitError] = useState<string | null>(null)

  useEffect(() => {
    if (university) {
      setName(university.name)
      setActive(university.status === 'نشط')
    }
  }, [university])

  if (!id) return null

  const handleConfirm = async () => {
    setSubmitting(true)
    setSubmitError(null)
    try {
      await updateUniversity(id, { name, status: active ? 'ACTIVE' : 'DISABLED' })
      onDataChanged()
      navigate(-1)
    } catch (err) {
      setSubmitError(err instanceof Error ? err.message : 'حدث خطأ، حاول مرة أخرى')
      setSubmitting(false)
    }
  }

  return (
    <Modal
      title="تعديل بيانات الجامعة"
      actions={
        <>
          <ModalButton variant="cancel">إلغاء</ModalButton>
          <ModalButton onClick={handleConfirm} disabled={submitting || !name.trim() || !university}>
            {submitting ? '...جاري الحفظ' : 'حفظ التعديلات'}
          </ModalButton>
        </>
      }
    >
      {loading ? (
        <CardSkeleton />
      ) : error || !university ? (
        <ErrorState description={error ?? 'تعذر العثور على الجامعة'} />
      ) : (
        <>
          <ModalField label="اسم الجامعة" value={name} onChange={setName} />
          <ModalToggleRow
            label="الحالة"
            value={active ? 'نشط' : 'معطل'}
            tone={active ? 'success' : 'danger'}
            defaultOn={active}
            onChange={setActive}
          />
          <ModalDangerZone
            label="حذف الجامعة"
            onClick={() => navigate(`/academic/universities/${id}/delete`)}
          />
          {submitError ? <p className="text-sm font-bold text-danger">{submitError}</p> : null}
        </>
      )}
    </Modal>
  )
}
